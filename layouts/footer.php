<footer>
    <div class="content-container-center">
        <div class="footer-content">
            <div class="footer-logo-container">
                <a href="/"><img src="<?php echo SITE_LINK; ?>images/footer-logo.jpg" alt="Company logo in the footer of the website."></a>
            </div>
            <div class="footer-main-link-container">
                <div class="footer-link-container">
                    <h4 class="white-text">Company</h4>
                    <ul class="footer-ul-01">
                        <li><a href="">About Us</a></li>
                        <li><a href="">Partners</a></li>
                        <li><a href="">Terms</a></li>
                    </ul>
                </div>
                <div class="footer-link-container">
                    <h4 class="white-text">Event Planners</h4>
                    <ul class="footer-ul-01">
                        <li><a href="">How It Works</a></li>
                        <li><a href="">Listings</a></li>
                    </ul>
                </div>
                <div class="footer-link-container">
                    <h4 class="white-text">Vendors</h4>
                    <ul class="footer-ul-01">
                        <li><a href="">Sign Up</a></li>
                        <li><a href="">Pricing</a></li>
                    </ul>
                </div>
            </div>
            <div class="footer-social-media-container">
                <div class="social-media">
                    <a href=""><img src="<?php echo SITE_LINK; ?>images/facebook-icon.png" alt="Facebook icon."></a>
                    <a href=""><img src="<?php echo SITE_LINK; ?>images/instagram-icon.png" alt="Instagram icon."></a>
                    <a href=""><img src="<?php echo SITE_LINK; ?>images/youtube-icon.png" alt="Youtube icon."></a>
                </div>
            </div>
        </div>
        <div class="copyright-container">
            <h5 class="white-text">&copy; AlylaTech, Inc 2022. All rights reserved.</h5>
            <h5 class="white-text">Event One Stop is a property of AlylaTech, Inc.</h5>
        </div>
    </div>
</footer> 